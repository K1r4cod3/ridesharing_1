<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        if (empty($usernameError) && empty($emailError) && empty($newPasswordError) && empty($confirmPasswordError)) {
            ...
            $checkQuery = "SELECT * FROM drivers WHERE username = '$username' AND email = '$email'";
            $checkResult = mysqli_query($conn, $checkQuery);

            if (mysqli_num_rows($checkResult) == 1) {
                // Update password
                $updateQuery = "UPDATE drivers SET password = '$newPassword' WHERE username = '$username' AND email = '$email'";
...
<body>
    <div class="border-b-2 border-gray-600 bg-[#ffddab]">
        <nav class="flex justify-between items-center max-w-[1240px] mx-auto py-4">
            ...
            <div>
                <a href="login.php" class="text-white bg-[#945034] rounded-lg px-4 py-2">Passenger Login</a>
    ...
    <div class="mx-auto max-w-[1000px] p-4 bg-[#ffddab] mt-10 rounded-lg">
        <h1 class="text-black text-4xl font-bold text-center">Driver Reset Password</h1>
        <form method="post" class="mx-auto flex flex-col gap-2 max-w-[400px] mt-10">
            ...
            <div class="flex gap-2 w-full">
                <button type="submit" class="bg-[#5F8B4C] rounded-lg px-4 py-4 w-full font-bold text-center text-white">Reset Password</button>
                <a href="driver_reset_password.php" class="bg-[#5F8B4C] rounded-lg px-4 py-4 w-full font-bold text-center text-white">Clear</a>
            </div>
        </form>
        <p class="text-center mt-5"><a href="driver_login.php" class="hover:underline hover:font-bold">Back to Login</a></p>
    </div>
</body>
</html> 